...
inline T load_relaxed(const T* p) noexcept
{
    static_assert(std::is_same_v<T, std::uint32_t> ||
                  ...
                  "portable_atomic only supports uint32_t and uint64_t");
...
static_assert(
    __atomic_always_lock_free(4, nullptr),
    "portable_atomic requires lock-free 32-bit atomics on this target");
static_assert(
    __atomic_always_lock_free(8, nullptr),
    "portable_atomic requires lock-free 64-bit atomics on this target");
...
#warning \
    "portable_atomic: Unsupported platform, falling back to non-atomic operations!"
