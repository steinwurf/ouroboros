...
inline T load_acquire(const T* p) noexcept
{
    ...
    // On x86/x64, loads are naturally acquire-ordered, but we add a compiler
    // barrier to ensure proper ordering semantics. This works with read-only
    // shared memory mappings.
    if constexpr (std::is_same_v<T, std::uint32_t>)
    {
        return impl::load_acquire_impl_u32(p);
    }
    else // T is uint64_t
    {
        return impl::load_acquire_impl_u64(p);
    }
}
...
inline void store_release(T* p, T v) noexcept
{
    ...
    if constexpr (std::is_same_v<T, std::uint32_t>)
    {
        impl::store_release_impl_u32(p, v);
    }
    else // T is uint64_t
    {
        impl::store_release_impl_u64(p, v);
    }
}
